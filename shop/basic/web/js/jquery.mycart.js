  ...
  var loadMyCartEvent = function(userOptions){
    ...
    if(!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '</div>' +
        '<div class="modal-footer">' + '<button id="checkout" ">checkout</button>'+'<form action="index.php?r=site/cart" method="post">' +
        '<input type="text" name="id" id="id" style="display: none"> ' +
        '<input type="submit" id="submit" style="display: none"> ' +
        '<input type="text" name="num" id="num" style="display: none">' +
        '</form>'+
    ...
    */
    ...
    $(document).on('click', "#checkout", function(){
      var obj = ProductManager.getAllProducts();
      var id = [];
      var num = [];
      for(var i=0;i<obj.length;i++){
        id.push(obj[i]['id']);
        num.push(obj[i]['quantity']);
      }
      console.log(id);
      $('#id').val(id);
      $('#num').val(num);
